...
- `img-src 'self'`, `font-src 'self' data:` — toutes les images (`/media/:id/file`, assets bundlés, `/mock/*.svg`) sont same-origin. Les polices (`@fontsource/*`, auto-hébergées) le sont aussi, **sauf un sous-ensemble unicode de `Manrope Variable`** que Vite inline en `data:font/woff2;base64,...` directement dans le CSS compilé (sous le seuil par défaut `assetsInlineLimit` de 4 Ko) — réel, généré par le build actuel, confirmé en inspectant `dist/client/_astro/global.*.css` ; `global.css` est importé par `BaseLayout` **et** `AdminLayout`, donc `data:` est nécessaire partout, pas seulement en admin. Aucun CDN externe utilisé nulle part dans le code.
...
- `script-src 'self'` + un `'sha256-...'` **calculé dynamiquement, par réponse**, pour chaque `<script type="module">` littéral que cette réponse contient réellement (+ `https://challenges.cloudflare.com` sur `/contact`/`/en/contact` uniquement, pour le loader Turnstile). **Jamais `'unsafe-inline'`, nulle part.** Voir "Correction Step 3 — régression CSP réelle" ci-dessous : l'hypothèse initiale ("Astro bundle tout `<script>` en fichier externe") était fausse, vérifiée directement dans le code compilé.
...
### Correction Step 3 — régression CSP réelle sur `/admin/media` (script-src / font-src)

La validation staging réelle du hardening initial (commit `bc7acc7`) a cassé "Ajouter des médias" sur `/admin/media` : Chrome bloquait le `<script>` qui appelle `initMediaUploader()` et un attribut `onchange="this.form.submit()"` sur le `<select>` de filtre, plus une police `data:font/woff2;base64,...`.

**Diagnostic, confirmé en inspectant `dist/server/` et `dist/client/` d'un vrai build, pas supposé** :
- `dist/client/_astro/` ne contient **aucun fichier `.js`** — l'hypothèse initiale ("Astro bundle chaque `<script>` en fichier externe") était fausse pour ce projet. `renderScript()` (`node_modules/astro/dist/runtime/server/render/script.js`) confirme pourquoi : sur une route on-demand (`prerender = false` — c'est le cas de tout ce site sauf `confidentialite`/`en/privacy`), un `<script>` non-`is:inline` est **toujours** rendu comme `<script type="module">CODE_LITTÉRAL</script>` directement dans le HTML de chaque réponse, avec aucun passage d'attribut (donc un `nonce` posé sur la balise source n'aurait jamais survécu — vérifié en lisant le code source d'Astro, pas supposé). C'est vrai partout, pas seulement en admin : `Header.astro`, `MobileNav.astro`, `ScrollReveal.astro` (utilisés sur **toutes** les pages publiques) ont exactement le même comportement, confirmé par une vraie requête `curl` contre `astro preview`.
- `onchange="this.form.submit()"` est un attribut d'écouteur d'événement inline — un mécanisme CSP différent (jamais couvert par un hash/nonce de `<script>`), corrigé en le retirant et en le câblant dans le `<script>` déjà présent sur cette page.
- Ni `tests/admin/routes.test.mjs` ni les suites Playwright ne l'avaient détecté : la première ne teste jamais le chemin `200` authentifié (pas de JWT Access réel disponible dans cet environnement), et les secondes tournent sous `astro dev`, où la CSP est volontairement désactivée (`import.meta.env.DEV`) — donc rien n'exerçait jamais réellement cette CSP contre une vraie page avant la validation staging.

**Correction — hash calculé par requête, jamais codé en dur.** `src/middleware.ts` lit le corps HTML de chaque réponse, calcule le `sha256` réel (base64) de chaque `<script type="module">` qu'elle contient (`crypto.subtle.digest`, natif Workers), et les ajoute à `script-src` de cette réponse précise. Aucune liste de hash à maintenir à la main : un futur `<script>` ajouté n'importe où — public ou admin — est automatiquement couvert, ce qui empêche structurellement cette classe de régression de se reproduire. `tests/admin/routes.test.mjs` vérifie maintenant, contre une vraie réponse HTTP, que chaque `<script>` d'une page est bien couvert par le hash présent dans la CSP de cette même réponse, et qu'aucun fichier `.astro` du dépôt ne contient plus d'attribut `onXxx="..."`.
