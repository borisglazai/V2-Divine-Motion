...
import { saveServiceAction } from "@/lib/admin/services-actions";

export const prerender = false;

/** "Save" only ever touches a draft — see saveServiceAction. */
export const POST: APIRoute = async ({ request, params, locals, redirect }) => {
  ...
  const result = await saveServiceAction(getDb(), idParam, formData, auth.identity.email);
