...
import { saveWorkItemAction } from "@/lib/admin/work-actions";

export const prerender = false;

/** "Save" only ever touches a draft (Brief 013 §14) — see saveWorkItemAction for the resolve-or-create-draft logic. */
export const POST: APIRoute = async ({ request, params, locals, redirect }) => {
  ...
  const result = await saveWorkItemAction(getDb(), idParam, formData, auth.identity.email);
